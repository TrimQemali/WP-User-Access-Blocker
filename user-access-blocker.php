 ...
 * Description: Block or unblock users from logging into WordPress. User Access Blocker is a WordPress plugin that provides administrators with the ability to selectively block and unblock users from logging into their WordPress site. This plugin offers a clean, user-friendly interface for managing user access without permanently deleting user accounts.
 * Version: 1.4.3
 * Plugin URI:  https://techcreative.dev/
 * Author: TechCreative
 * Author URI:  https://techcreative.dev/
 * License:     GPL-3.0+
 * Text Domain: wp-user-access-blocker
...
class User_Access_Blocker {
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo esc_html(date('F j, Y, g:i a', $blocked_data['timestamp'])); ?></td>
                                    <td>
    ...
    public function ajax_block_user() {
        // Check nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'user_blocker_nonce')) {
        ...
        wp_send_json_success(array(
            ...
            'user' => array(
                ...
                'blocked_on' => date('F j, Y, g:i a', time()),
            ),
        ));
    }
    ...
    public function check_if_blocked($user, $username, $password) {
        ...
        if (isset($blocked_users[$user->ID])) {
            return new WP_Error(
                'blocked_user',
                __('Your account has been blocked. Please contact the site administrator for assistance.')
            );
        }
